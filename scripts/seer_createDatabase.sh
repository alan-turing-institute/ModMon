...
do
  ...
  sed -i '1s/ /_/g' seer_${fn}_breastCancer_pk.csv
  # USE FIRST 20 LINES TO GENERATE CREATE STATEMENT
  head -n 20 seer_${fn}_breastCancer_pk.csv |csvsql > seer_${fn}_create.sql
  # SUBSTITUTE stdin WITH seer????
  sed -i 's/stdin/seer${fn}/' seer_${fn}_create.sql
  # REMOVE NOT NULLs
  sed -i 's/NOT NULL//g' seer_${fn}_create.sql
  # SUBSTITUTE Blank(s) WITH NULL
  sed -i 's/Blank(s)/NULL/g' seer_${fn}_breastCancer_pk.csv
  # REMOVE QUOTES
  sed -i 's/"NULL"/NULL/g' seer_${fn}_breastCancer_pk.csv
  # SUBSTITUTE Unknown WITH NULL
  sed -i 's/,Unknown,/,NULL,/g' seer_${fn}_breastCancer_pk.csv
  # SUBSTITUTE "Unknown" WITH NULL
  sed -i 's/,"Unknown",/,NULL,/g' seer_${fn}_breastCancer_pk.csv
done
